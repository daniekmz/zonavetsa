...
import { Trophy, Star, Award, TrendingUp, Search, Users, Globe, Sparkles, BookOpen, ClipboardCheck, Activity, BarChart3, Briefcase } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-3 py-4 sm:px-4 lg:px-6">
      ...
      <div className="panel-surface relative overflow-hidden bg-gradient-to-br from-primary via-sky-700 to-cyan-600 p-5 text-white sm:p-8">
        ...
        <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h2 className="text-2xl font-black sm:text-4xl">Peringkat Siswa</h2>
          ...
          <div className="w-full space-y-3 lg:w-auto">
            ...
            <div className="grid grid-cols-3 gap-3 rounded-2xl border border-white/20 bg-white/10 p-3 backdrop-blur-sm">
              <div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-sky-100/70">Siswa</p>
                <p className="mt-1 text-2xl font-black">{summary.totalStudents}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-sky-100/70">Rata-rata</p>
                <div className="mt-1 flex items-center gap-1">
                  <Star size={14} className="fill-amber-300 text-amber-300" />
                  <p className="text-2xl font-black">{summary.average}</p>
                </div>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-sky-100/70">Ujian Max</p>
                <p className="mt-1 text-2xl font-black">{summary.highestExam}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:flex-row sm:items-center">
        ...
        <div className="glass-tech inline-flex items-center gap-2 rounded-xl p-1">
      ...
      {filteredStudents.length === 0 ? (
        <div className="rounded-3xl border-2 border-dashed border-slate-300 bg-white dark:bg-slate-900 p-10 text-center dark:border-slate-700">
              ...
              >
                ...
                >
                  ...
                  <div className="mt-4 flex items-center gap-3">
                    <div className="relative h-14 w-14 overflow-hidden rounded-2xl border border-white/70 bg-white dark:bg-slate-900 shadow-sm dark:border-slate-700">
      ...
      <div className="relative z-10 space-y-3 pt-2">
        ...
        {(searchQuery ? filteredStudents : others).length > 0 ? (
          (searchQuery ? filteredStudents : others).map((student, idx) => {
            ...
            return (
              <motion.div
                ...
                className="relative flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 p-4 shadow-sm transition-colors sm:flex-row sm:items-center sm:justify-between dark:border-slate-800"
              >
                <div className="flex min-w-0 items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-sm font-black text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                    {actualRank}
                  </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white dark:bg-slate-900 p-6 text-center dark:border-slate-800">
